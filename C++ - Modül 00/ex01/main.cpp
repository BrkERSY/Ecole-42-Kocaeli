...
void    Search(PhoneBook *str){
    ...
    std::cout << "\033[32mKayıtlı Kişiler\n\033[0m" << std::endl;
	...
	std::istringstream my_stream(temp_int); // Listemele sonrası 2 kez Seçim yaz basmayı düzeltiyor. Stringin integer çevrilip çevrilmediğine göre işlem yapıyor.
    if(!(my_stream >> kisiNo))
	{
    ...
    if (i >= 1 && i <= 8 && !(str->kisiler[kisiNo].ad.empty()) && !(str->kisiler[kisiNo].soyad.empty()) && !(str->kisiler[kisiNo].ad.empty()))
    {
...
void    Add(PhoneBook *str){
    ...
    if (str->kisiler[str->count].ad.empty() || str->kisiler[str->count].soyad.empty() || \
        str->kisiler[str->count].takmaAd.empty())
        return ;
